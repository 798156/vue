  ...
  <v-card>
    <v-card-item class="py-4">
      <div class="d-flex align-center justify-space-between">
        <h4 class="text-18 font-weight-medium">添加支付方式</h4>
    ...
    <v-card-text>
      ...
      <v-row>
        ...
        <v-col cols="12">
          <v-text-field v-model="textcno" label="Card Number" required color="primary" variant="outlined" hide-details></v-text-field>
        </v-col>
        <v-col cols="12" sm="6">
          <v-text-field v-model="textdate" label="Expiry Date" required color="primary" variant="outlined" hide-details></v-text-field>
        </v-col>
        <v-col cols="12" sm="6">
    ...
    <v-card-actions class="justify-end">
      <v-btn color="error" @click="$emit('dialogcard')">取消</v-btn>
      <v-btn color="primary" variant="flat" @click="$emit('dialogcard')">递交</v-btn>
    </v-card-actions>
  </v-card>
</template>
